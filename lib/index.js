...
const expect = function (value) {
  return {
    toEqual: function (expected) {
      if (value === expected) {
        itResults.push({name: `Expected ${value} toEqual ${expected}`, status: true});
        passedTests ++;
      } else {
        itResults.push({name: `Expected ${value} toEqual ${expected}`, status: false});
        failedTests ++;
      }
    },
...
const showTestResults = function() {
  ...
  itResults.forEach(result => {
    console.log(result.name, result.status ? '✔︎ Passed!' : 'Ⓧ Failed');
  });
}
